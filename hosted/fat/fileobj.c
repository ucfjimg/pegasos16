...
int fobj_create(fatvol_t *fv, const char *fn, uint8_t attr, fileinfo_t *info, clusidx_t *parent)
{
     ...
     */
    CHECK_RET(bc_get(fv->dev, fv->unit, info->sector, &blk, 0));

	ent = (dirent_t __far *)(blk->data + info->secoffs);
...
/* Given information about a file, and an offset withing the file, return
...
)
{
    ...
    while (clus--) {
        ...
        if (ci >= 0xff0) { /* TODO FATx */ 
            /* premature EOF
             */
            fprintf(stderr, "trunc: premature EOF\n");
