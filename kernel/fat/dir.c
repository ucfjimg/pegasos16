...
int dir_find_path(fatvol_t *fv, char *fn, fileinfo_t *info)
{
    ...
    if (ss.len == 0) {
        if (fn[0] == '/') {
            /* open root directory. we have to construct this
             * as FAT doesn't store a dir entry for root on disk.
             */
            info->ino = 0;
            memset(&info->dir, ' ', sizeof(info->dir));
            info->dir.attr = FA_DIR;
            info->dir.major = 0;
            info->dir.minor = 0;
            memset(&info->dir.reserved, 0, sizeof(info->dir.reserved));
            info->dir.moddate = 0;
            info->dir.modtime = 0;
            /* by convention cluster 0 refers to the contents of the 
             * root directory
             */
            info->dir.cluster0 = 0;
            info->dir.size = 0;
...
int dir_find_ent(fatvol_t *fv, char *path, int len, clusidx_t cluster, fileinfo_t *info)
{
    ...
    for(;;) {
        ...
        if (
			...
			info->ino = FAT_INO(blk->blkno, offs);
            info->dir = *dirent;
